...
while true; do
    read -s -p "Please enter your sudo password for unattended installation: " SUDO_PASSWORD
    echo
...
# usage:
# my_command & spinner "message while waiting"


# Display a zenity message with self-destruction
TitleMSG="no message"
MSG="no message"

show_progress_message() {
    local title="$TitleMSG"
    local message="$MSG"

    (
        for i in {1..100}; do
            echo $i
            sleep 0.1  # self-destruct speed control
        done
    ) | zenity --progress --title="$title" --text="$message" --percentage=0 --auto-close --no-cancel

    sleep 0.3 # ensurance that zenity has closed
}
...
if [ ${#NEW_PACKAGES[@]} -gt 0 ]; then
  ...
  sleep 0.4
  if [ "$PKG_MANAGER" = "apt" ]; then
echo "$SUDO_PASSWORD" | sudo -S apt install "${NEW_PACKAGES[@]}" -y
  elif [ "$PKG_MANAGER" = "pacman" ]; then
echo "$SUDO_PASSWORD" | sudo -S pacman -Syu "${NEW_PACKAGES[@]}" --noconfirm
  elif [ "$PKG_MANAGER" = "dnf" ]; then
echo "$SUDO_PASSWORD" | sudo -S dnf install "${NEW_PACKAGES[@]}" -y
  fi
...
echo "$SUDO_PASSWORD" | sudo -S cp $HOME/affinity_setup_tmp/rum/rum /usr/local/bin/rum

# Download then extract custom wine binaries specifically made to run Affinity apps better.
echo "Downloading Wine:"
aria2c $ARIA2_PARAMETERS --out ElementalWarrior-wine.7z  https://github.com/woafID/LinuxCreativeSoftware/releases/download/wine10.3/ElementalWarrior-wine.7z
...
echo "$SUDO_PASSWORD" | sudo -S mkdir -p "/opt/wines"
echo "$SUDO_PASSWORD" | sudo -S cp --recursive "$HOME/affinity_setup_tmp/ElementalWarrior-wine/wine-install" "/opt/wines/ElementalWarrior-8.14"

# Link wine to fix an issue because it does not have a 64bit binary.
echo "$SUDO_PASSWORD" | sudo -S ln -s /opt/wines/ElementalWarrior-8.14/bin/wine /opt/wines/ElementalWarrior-8.14/bin/wine64

TitleMSG="Wine Mono Notice"
MSG="You may get prompted to install Wine Mono, in the next section. Please proceed with installing it. Other parts of the installation will be silent. Be patient."
show_progress_message
...
aria2c $ARIA2_PARAMETERS --out Windows.winmd https://github.com/microsoft/windows-rs/raw/master/crates/libs/bindgen/default/Windows.winmd
mkdir -p $HOME/LinuxCreativeSoftware/Affinity/drive_c/windows/system32/WinMetadata
cp -f $HOME/affinity_setup_tmp/Windows.winmd $HOME/LinuxCreativeSoftware/Affinity/drive_c/windows/system32/WinMetadata/Windows.winmd &>/dev/null
...
TitleMSG="Install Notice"
MSG="Please proceed with all of the installers."
show_progress_message
...
aria2c $ARIA2_PARAMETERS --out wintypes.dll https://github.com/ElementalWarrior/wine-wintypes.dll-for-affinity/raw/refs/heads/master/wintypes_shim.dll.so &>/dev/null &
spinner "Downloading wintypes fix"
cp -f $HOME/affinity_setup_tmp/wintypes.dll $HOME/LinuxCreativeSoftware/Affinity/drive_c/Program\ Files/Affinity/Designer\ 2/
cp -f $HOME/affinity_setup_tmp/wintypes.dll $HOME/LinuxCreativeSoftware/Affinity/drive_c/Program\ Files/Affinity/Photo\ 2/
cp -f $HOME/affinity_setup_tmp/wintypes.dll $HOME/LinuxCreativeSoftware/Affinity/drive_c/Program\ Files/Affinity/Publisher\ 2/
...
aria2c --console-log-level=warn --dir $HOME/LinuxCreativeSoftware/Affinity/drive_c/launchers/icos/ --out designer2.svg https://cdn.serif.com/affinity/img/global/logos/affinity-designer-2-020520191502.svg &>/dev/null
echo "Creating Photo icon..."
aria2c --console-log-level=warn --dir $HOME/LinuxCreativeSoftware/Affinity/drive_c/launchers/icos/ --out photo2.svg https://cdn.serif.com/affinity/img/global/logos/affinity-photo-2-020520191502.svg &>/dev/null
echo "Creating Publisher icon..."
aria2c --console-log-level=warn --dir $HOME/LinuxCreativeSoftware/Affinity/drive_c/launchers/icos/ --out publisher2.svg https://cdn.serif.com/affinity/img/global/logos/affinity-publisher-2-020520191502.svg &>/dev/null
...
Icon=$HOME_DIR/LinuxCreativeSoftware/Affinity/drive_c/launchers/icos/designer2.svg
...
Icon=$HOME_DIR/LinuxCreativeSoftware/Affinity/drive_c/launchers/icos/photo2.svg
Categories=ConsoleOnly;System;"
...
Icon=$HOME_DIR/LinuxCreativeSoftware/Affinity/drive_c/launchers/icos/publisher2.svg
Categories=ConsoleOnly;System;"
